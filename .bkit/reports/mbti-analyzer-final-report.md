...
```
549f5b2  성능 최적화: 이미지 5장 압축 전처리, Gemini 타임아웃 55초 연장, maxDuration=60 설정, 로딩 스텝 이미지별 세분화
```
...
| 파일 | 변경 내용 |
|------|-----------|
| `src/lib/image-utils.js` | 장수별 압축 프리셋 추가 (1장: 1024px/82% → 5장: 640px/62%) |
| `src/lib/gemini.js` | TIMEOUT_MS 30000 → 55000, `candidateCount: 1` 추가 |
| `src/app/api/analyze/route.js` | `export const maxDuration = 60` 추가 |
| `src/constants/loading-steps.js` | 이미지별 동적 단계 생성 (`N번 이미지 분석 중...`) |
| `src/hooks/useAnalysis.js` | `imageCount` 노출, 압축 시 `totalImages` 전달, 인터벌 동적 조정 |
| `src/components/LoadingScreen.jsx` | `imageCount` prop을 `getLoadingSteps`에 전달 |
...
## 9. 이미지 5장 타임아웃 해결 상세 (2차 Iterate)

### 9.1 문제 원인 분석

```
원인 1: 클라이언트 → 서버 페이로드 과대
  - 5장 × 1024px × JPEG 80% ≈ 약 8~12MB 전송
  - 네트워크 업로드 지연 → 서버 수신 전 클라이언트 타임아웃

원인 2: Vercel 기본 함수 실행 시간 제한
  - Next.js API Route 기본 maxDuration = 10초 (Hobby 플랜)
  - Gemini 5장 처리 시간 ≈ 20~40초 → 타임아웃 발생

원인 3: AbortController 타임아웃 30초
  - Gemini 처리 완료 전에 클라이언트가 요청 중단
```

### 9.2 해결 전략

```
전략 1: 클라이언트 이미지 압축 강화
  - 장수별 프리셋: 5장 기준 640px / JPEG 62%
  - 5장 총 페이로드: ~12MB → ~3.5MB (약 70% 감소)

전략 2: Vercel maxDuration 확장
  - export const maxDuration = 60 (Hobby: 최대 60초)

전략 3: Gemini 타임아웃 연장
  - TIMEOUT_MS: 30000 → 55000 (maxDuration보다 5초 여유)
  - candidateCount: 1 (불필요한 후보 생성 제거)

전략 4: 로딩 UX 개선
  - 이미지별 단계 표시로 사용자 체감 대기시간 감소
  - 인터벌 동적 조정: 5장 기준 3000ms/단계
```

### 9.3 압축 프리셋 상세

| 장수 | 최대 해상도 | JPEG 품질 | 예상 파일 크기 |
|------|------------|-----------|---------------|
| 1장 | 1024px | 82% | ~300KB |
| 2장 | 900px | 78% | ~240KB |
| 3장 | 800px | 72% | ~200KB |
...
> MBTI 분석에 필요한 텍스트 가독성은 640px에서도 충분히 유지됩니다.
