...
- **수익화**: **Free(빠른 추정)** — 디바이스당 무료 **3회**(`FREE_LIMIT`, `analysis-tier.js`) 후 Premium 탭·결제 유도; **Premium** — 매회 유료(₩1,900)
- **보안**: 이미지 Base64 변환 후 즉시 폐기, 서버·DB에 이미지 파일 저장 금지

### 1.3 사용자 여정 (Critical Path)

```
[1] 데이터 입력(Free·Premium 탭) → [2] 무료 횟수 조회(Supabase) → [3] 분기 처리
       ├─ Premium: 항상 포트원 결제 → 성공 시 AI 풀 리포트
       └─ Free: 누적 < 3회 → 즉시 AI 짧은 추정 | 누적 ≥ 3회 → Premium 탭 전환·결제 → 풀 리포트
...
| F-202 | 간단 모드 무료 상한·유료 분기 | `FREE_LIMIT`(3) 미만: 무료, 이상: 결제창; 심층은 항상 결제 (`analysis-tier.js`) | P0 |
...
10. [F-202] 간단 모드 무료 상한(3) / 유료 분기·심층 결제
